...
from pcdet.utils.loss_utils import ChamferDistance
from .rbgnet_utils import BasicBlock1D
...
class VoteModule(nn.Module):
    ...
    def __init__(self, model_cfg):
        ...
        self.vote_per_seed = model_cfg['VOTE_PER_SEED']
        self.gt_per_seed = model_cfg['GT_PER_SEED']
        self.conv_channels = model_cfg['CONV_CHANNELS']
        self.num_points = model_cfg.get('NUM_POINTS', -1)
        self.norm_feats = model_cfg['NORM_FEATS']
        self.with_res_feat = model_cfg.get('WITH_RES_FEAT', True)
        self.vote_xyz_range = model_cfg.get('VOTE_XYZ_RANGE', None)
        # assert vote_xyz_range is None or is_tuple_of(vote_xyz_range, float)
        self.vote_loss_cfg = model_cfg.get('VOTE_LOSS', None)
        if self.vote_loss_cfg['NAME'] == 'ChamferDistance':
            self.vote_loss = ChamferDistance(mode=self.vote_loss_cfg['MODE'], reduction='none', loss_dst_weight=self.vote_loss_cfg['LOSS_DST_WEIGHT'])
        else:
            raise NotImplementedError

        prev_channels = copy.deepcopy(self.in_channels)
        vote_conv_list = list()
