...
def d3_box_overlap_kernel(boxes, qboxes, rinc, criterion=-1):
    ...
    for i in range(N):
        for j in range(K):
            if rinc[i, j] > 0:
                boxes1_top_height = boxes[i, 2] + boxes[i, 5] / 2.
                boxes1_bottom_height = boxes[i, 2] - boxes[i, 5] / 2.
                boxes2_top_height = qboxes[j, 2] + qboxes[j, 5] / 2.
                boxes2_bottom_height = qboxes[j, 2] - qboxes[j, 5] / 2.

                heighest_of_bottom = max(boxes1_bottom_height,
                                       boxes2_bottom_height)
                lowest_of_top = min(boxes1_top_height, boxes2_top_height)
                iw = max(lowest_of_top - heighest_of_bottom, 0)
...
def eval_det_cls(pred, gt, iou_thr=None):
    ...
    for img_id in gt.keys():
        ...
        if cur_gt_num != 0:
            gt_cur = np.zeros((cur_gt_num, 7), dtype=np.float32)
    ...
    for img_id in pred.keys():
        ...
        if cur_num == 0:
            continue
        pred_cur = np.zeros((cur_num, 7), dtype=np.float32)
        box_idx = 0
        ...
        if len(gt_cur) > 0:
            iou_cur = d3_box_overlap(pred_cur, gt_cur)          
...
def indoor_eval(gt_annos,
    ...
    for img_id in range(len(dt_annos)):
        ...
        for i in range(len(det_anno['labels_3d'])):
            label = det_anno['labels_3d'][i]  # [N,]
            bbox = det_anno['boxes_3d'][i]  # [N, 7]
            score = det_anno['scores_3d'][i]  # [N,]
        ...
        if gt_anno['gt_num'] != 0:
            if gt_anno['gt_boxes_upright_depth'].shape[-1] == 6:
                fake_rot = np.zeros((gt_anno['gt_boxes_upright_depth'].shape[0], 1), dtype=np.float32)
                gt_boxes = np.concatenate((gt_anno['gt_boxes_upright_depth'].copy(), fake_rot), axis=-1)
            elif gt_anno['gt_boxes_upright_depth'].shape[-1] == 7:
                gt_boxes = gt_anno['gt_boxes_upright_depth'].copy()
            else:
